  ...
  return (
    <div className="space-y-5 max-w-[1400px]">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        {[
          { label: 'Available Beds', value: available, color: 'text-emerald-400' },
          { label: 'Occupied Beds', value: occupied, color: 'text-indigo-400' },
          { label: 'Maintenance', value: maintenance, color: 'text-amber-400' },
          { label: 'Total Equipment', value: equipment?.length ?? 0, color: 'text-cyan-400' },
          { label: 'Total Staff', value: staff?.length ?? 0, color: 'text-violet-400' },
          { label: 'Available Staff', value: staff?.filter((s) => s.available).length ?? 0, color: 'text-emerald-400' },
        ].map((s, i) => (
          <motion.div
            key={s.label}
